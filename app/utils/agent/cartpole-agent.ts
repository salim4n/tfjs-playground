...
class RLAgent {
  ...
  async train(
    ...
    const target =
      ...
      (1 - +done) *
        this.gamma *
        (this.targetModel.predict(tf.tensor2d([nextState])) as tf.Tensor)
          .max()
          .dataSync()[0]

    const targetF = (
      this.model.predict(tf.tensor2d([state])) as tf.Tensor
    ).dataSync()
    targetF[action] = target

    const history = await this.model.fit(
      tf.tensor2d([state]),
      tf.tensor2d([targetF as unknown as number[]])
    )
    this.lossValues.push({
      batch: this.lossValues.length,
      loss: history.history.loss[0] as number,
    })
    tfvis.show.history(
      tfvis.visor().surface({ name: "Training Loss", tab: "Training" }),
      this.lossValues,
      ["loss"]
    )
    ...
    tf.dispose([this.lossValues, nextState, done])
    tf.dispose([state, action, reward])
  }
  ...
  async act(state: number[]): Promise<number> {
    if (Math.random() < this.epsilon) {
      ...
      return ((await this.model.predict(tf.tensor2d([state]))) as tf.Tensor)
        .argMax(1)
        .dataSync()[0]
    }
  }
